...
// Intercepte les liens de partage Google Drive et les transforme en lien
// d'affichage direct (https://lh3.googleusercontent.com/d/ID), qui reste
// fiable pour l'affichage en balise <img> ou en fond d'écran plein écran.
function convertImageUrl(url) {
  if (!url) return '';
  url = url.trim();

  // Lien de partage classique : drive.google.com/file/d/ID/view?usp=sharing
  const driveFileMatch = url.match(/drive\.google\.com\/file\/d\/([a-zA-Z0-9_-]+)/);
  // Lien du type drive.google.com/open?id=ID ou drive.google.com/uc?id=ID
...
async function loadData() {
  try {
    ...
    if (settingsRes.ok) {
      ...
      data.settings = { ...s, logoUrl: convertImageUrl(s.logoUrl), heroImage: convertImageUrl(s.heroImage) };
    }
...
// ─── PROMOTION (prix barré → pourcentage de réduction) ───
...
// ─── HERO IMAGE ───
// Priorité : image Hero définie dans l'admin (settings.heroImage).
// Si aucune n'est définie, on retombe sur la photo du premier produit.
function setHeroImage() {
  const heroSrc = data.settings.heroImage;
  const prods = data.products.filter(p => p.image);
  const src = heroSrc || (prods.length > 0 ? prods[0].image : '');
  const alt = heroSrc ? 'House of Meila' : (prods[0] && prods[0].name) || 'House of Meila';

  if (src) {
    const c = document.getElementById('hero-img-container');
    c.innerHTML = `<img src="${src}" alt="${alt}" style="width:100%;height:100%;object-fit:cover" onerror="this.parentNode.innerHTML='<div class=hero-img-placeholder><svg viewBox=\\'0 0 24 24\\' fill=\\'none\\' stroke=\\'currentColor\\' stroke-width=\\'1\\'><path d=\\'M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z\\'/></svg><p>Bijoux House of Meila</p></div>'">`;
  }
}
...
function productCard(p) {
  ...
  const discountBadge = discount > 0 ? `<div class="product-card-discount">-${discount}%</div>` : '';
  const soldOutOverlay = isSoldOut ? `<div class="product-card-soldout-overlay"><span class="product-card-soldout-badge">Épuisé</span></div>` : '';
  const oldPriceHtml = discount > 0 ? `<span class="product-card-oldprice">${formatPrice(p.oldPrice)}</span>` : '';
  const waBtnHtml = isSoldOut
    ? `<span class="btn-whatsapp is-disabled">Indisponible</span>`
  ...
  return `
  <div class="product-card${isSoldOut ? ' is-soldout' : ''}" onclick="openModal('${p.id}')">
    <div class="product-card-img${isSoldOut ? ' is-soldout' : ''}">
      ${imgHtml}
      ${discountBadge}
      ${soldOutOverlay}
...
function openModal(id) {
  ...
  const discountBadge = discount > 0 ? `<div class="modal-discount">-${discount}%</div>` : '';
  const soldOutOverlay = isSoldOut ? `<div class="modal-soldout-overlay"><span class="modal-soldout-badge">Épuisé</span></div>` : '';
  const oldPriceHtml = discount > 0 ? `<span class="modal-oldprice">${formatPrice(p.oldPrice)}</span>` : '';
  const waActionHtml = isSoldOut
    ? `<div class="btn-wa-modal is-disabled" style="pointer-events:none;opacity:.5">Produit épuisé</div>`
  ...
  document.getElementById('modal-box').innerHTML = `
    <div class="modal-img">${imgHtml}${discountBadge}${soldOutOverlay}</div>
    <div class="modal-body">
      <div class="modal-cat">${getCatLabel(p.category || '')}</div>
      <h2 class="modal-name">${p.name}</h2>
      <div class="modal-price">${oldPriceHtml}${formatPrice(p.price)} <span>HTG</span></div>
...
function renderSocialSection() {
  ...
  grid.innerHTML = items.map(item => `
    <a class="social-circle-item reveal" href="${item.url}" target="_blank" rel="noopener">
...
// Ajoute une classe .in-view aux éléments .reveal dès qu'ils entrent dans
// le viewport, pour une apparition douce (fondu + léger décalage vertical).
// Défini une seule fois ; réutilisable après chaque re-rendu dynamique
// (catalogue, réseaux sociaux) via observeReveals().
